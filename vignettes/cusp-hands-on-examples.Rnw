...
The columns of \code{zeeman1} respectively contain the value on the asymmetry axis which is orthogonal to the central axis (\code{x}), the value of the bifurcation variable which is parallel to the central axis (\code{y}), and state variable, the shortest distance from the wheel strap point to the central axis.
